...
public class SwerveModuleNeo extends SwerveModule {
    ...
    protected void applySwerveModuleState(double velocityMPS, Rotation2d angleRadians) {
        angleMotor.getClosedLoopController().setSetpoint(angleRadians.getRotations(), SparkMax.ControlType.kPosition);
        driveMotor.getClosedLoopController().setSetpoint(velocityMPS, SparkMax.ControlType.kVelocity);
    }
    ...
    public void runSysIdRoutine(double voltage) {
        angleMotor.getClosedLoopController().setSetpoint(0.0, SparkMax.ControlType.kPosition);
